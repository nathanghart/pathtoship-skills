...
1. Make sure the work is committed and pushed (the scan reads GitHub). If the repository is private and `start_scan` returns `INSTALL_REQUIRED`, show the `install_url`, ask the user to install PathToShip for that repository, then retry.
